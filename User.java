...
public class User {
    ...
    public boolean getUserLogin(String username, String passsword) {
        try {
            ...
            String query = "SELECT * FROM credentials WHERE username = ?"; // Complete this
    ...
    public void createUserAndCredentials(String username, String password, String first_name, String last_name) {
        ...
        try {
            Connection con = conn.getConnection();
            String query = "SELECT COUNT(*) FROM credentials WHERE username = ?"; // 3. Complete this query, tip use COUNT()
            ...
            if (!isExist) {
                query = "INSERT INTO credentials(username, password) VALUES(?,?)"; // 4. Complete this query
                ...
                if (res.next()) {
                    ...
                    String newQuery = "INSERT INTO users(id, first_name, last_name) VALUES(?,?,?)";
    ...
    public void deleteUser(String id) {
        try {
            Connection con = conn.getConnection();
            String credQuery = "UPDATE credentials SET username=?,password=? WHERE id=?"; // 6. Complete this query
    ...
    public void updateUserCredential(String id, String first_name, String last_name) {
        try {
            ...
            String credQuery = "DELETE FROM users WHERE id = ?";
            PreparedStatement credStmt = con.prepareStatement(credQuery);
    ...
    public boolean getUser(String id){
        try {
            Connection con = conn.getConnection();
            String query = "UPDATE users SET first_name=?, last_name=? WHERE id=?"; // 9. Complete this query, you can do it.
            PreparedStatement stmt = con.prepareStatement(query);
            stmt.setString(1, id);
            ResultSet res = stmt.executeQuery();
            if(res.next()){
